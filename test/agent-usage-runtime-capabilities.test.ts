...
describe("Runtime capability evidence", () => {
  it("excludes live MCP mirrors with Unicode and spaces in their structured names", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "unicode-live", kind: "execute", status: "completed",
      rawInput: { server: "项目 工具", tool: "搜索 内容", arguments: {} }, rawOutput: "结果" });
    expect(h.collector.attribution.invocations()).toEqual([]);
  });

  it("restores historical MCP names containing Unicode and spaces without changing their identity", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "unicode-history", kind: "execute", status: "completed",
      rawInput: { server: "项目 工具", tool: "搜索 内容", arguments: {} }, rawOutput: "结果" },
  ...
  it.each(["full", "sparse"])("waits for terminal MCP output and retains pending estimates across restart (%s)", async mode => {
    const h = harness();
    wrapperCall(h, "streamed-wrapper");
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "streamed", kind: "execute", status: "in_progress",
    ...
    await restarted.runtimeCapabilities.recordTool(h.runId, { toolCallId: "streamed", status: "completed",
  ...
  it.each(["recreated", "renamed"])("does not duplicate a historical wrapper after its server was %s", async change => {
    const h = harness();
    ...
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "old-name", status: "completed",
  ...
  it("does not treat current server configuration as confirmed historical identity without wrapper evidence", async () => {
    const h = harness();
    h.db.prepare(`INSERT INTO agent_mcp_servers (id,agent_id,name,transport,enabled,created_at,updated_at)
      VALUES (8,?,'project-tools','stdio',1,'2026-09-22T00:00:00Z','2026-09-22T00:00:00Z')`)
      .run(h.collector.binding(h.sessionId).agentId);
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "unknown-history", status: "completed",
  ...
  it("does not serialize a live MCP mirror output that is excluded from Runtime accounting", async () => {
    const h = harness();
    let serialized = 0;
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "unmeasured", status: "completed",
      rawInput: { server: "7", tool: "search", arguments: {} },
      rawOutput: { toJSON: () => { serialized++; return "large output"; } } });
    expect(serialized).toBe(0);
    expect(h.collector.attribution.invocations()).toEqual([]);
  });
  it("freezes a Run before its first tool event and does not move established timestamps on replay", async () => {
    const h = harness();
    ...
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "frozen", kind: "read", status: "completed", rawOutput: "final result" },
      { eventId: 20, sequence: 20, occurredAt: "2026-09-21T01:00:01.000Z" });
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "frozen", kind: "read", status: "in_progress",
  ...
  it("recovers a legacy Run epoch from existing invocations without the new Run binding table", async () => {
    const h = harness();
    wrapperCall(h, "legacy-wrapper");
    h.db.prepare("UPDATE agent_usage_subjects SET epoch = epoch + 1 WHERE kind = 'session'").run();
    const restarted = new HostUsageCollector(h.db);
    await restarted.runtimeCapabilities.recordTool(h.runId, { toolCallId: "legacy-read", kind: "read", status: "completed" },
  ...
  it("does not pair a mirror with a different-time wrapper or reuse an already claimed wrapper", async () => {
    const h = harness();
    ...
    for (const [toolCallId, occurredAt] of [["far", "2026-09-21T02:00:00.000Z"],
      ...
      await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId, kind: "execute", status: "completed",
  ...
  it("enriches one original MCP wrapper across live mirror, sparse historical replay and restart", async () => {
    const h = harness();
    ...
    await h.collector.runtimeCapabilities.recordTool(h.runId, start);
    ...
    await restarted.runtimeCapabilities.recordTool(h.runId, terminal, event);
    await restarted.runtimeCapabilities.recordTool(h.runId, terminal, event);
  ...
  it("restores historical MCP calls without wrapper evidence using original date and stable epoch", async () => {
    const h = harness();
    ...
    await h.collector.runtimeCapabilities.recordTool(h.runId, content, event);
    h.db.prepare("UPDATE agent_usage_subjects SET epoch = epoch + 1 WHERE kind = 'session'").run();
    const restarted = new HostUsageCollector(h.db);
    await restarted.runtimeCapabilities.recordTool(h.runId, content, event);
  ...
  it("does not assign concurrent ambiguous MCP output or create a duplicate fallback", async () => {
    const h = harness();
    wrapperCall(h, "wrapper-one"); wrapperCall(h, "wrapper-two");
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "ambiguous", kind: "execute", status: "completed",
  ...
  it("attributes shell reads, reference reads and interpreter scripts to Skill and plugin", async () => {
    const h = harness();
    ...
    for (const [index, command] of commands.entries()) await h.collector.runtimeCapabilities.recordTool(h.runId, {
  ...
  it("replays terminal shell history on its original epoch and dates after reset and restart", async () => {
    const h = harness();
    const skill = projectedSkill(h.sessionId);
    h.collector.runtimeCapabilities.recordProjection(h.runId, [skill]);
    await h.collector.runtimeCapabilities.recordTool(h.runId, {
    ...
    const restarted = new HostUsageCollector(h.db);
    const replay = async () => {
      await restarted.runtimeCapabilities.recordTool(h.runId, {
  ...
  it("does not count structured MCP execute events or sparse updates as CLI", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, {
      toolCallId: "mcp-search", kind: "execute", status: "in_progress",
      rawInput: { server: "project-tools", tool: "search", arguments: { query: "private" } }
    });
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "mcp-search", status: "completed" });
    expect(ranks(h.collector, "cli")).toEqual([]);
    expect(ranks(h.collector, "unknown")).toEqual([]);
  });

  it("requires command evidence and upgrades a sparse unknown classification", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "unknown", kind: "execute", status: "completed" });
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "command", status: "pending" });
    await h.collector.runtimeCapabilities.recordTool(h.runId, {
      toolCallId: "command", kind: "execute", status: "in_progress", rawInput: { command: "echo hello" }
    });
    await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId: "command", status: "completed" });
  ...
  it("counts a SKILL.md read once across running and completed updates and distinguishes references", async () => {
    const h = harness();
    const skill = projectedSkill(h.sessionId);
    h.collector.runtimeCapabilities.recordProjection(h.runId, [skill]);
  ...
  it("keeps same-named builtin tools separate across Runtime providers", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, {
    ...
    await h.collector.runtimeCapabilities.recordTool(runId, {
      toolCallId: "read-1", kind: "read", status: "completed", rawInput: { path: "README.md" }
    });
  ...
  it("records only the structured outer executable and keeps composite shell commands opaque", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, {
  ...
  it("attributes structured Skill scripts to overlapping Skill and plugin views", async () => {
    const h = harness();
    const skill = projectedSkill(h.sessionId, {
      source: "plugin",
  ...
  it("finishes persisted Skill and plugin associations from sparse terminal updates", async () => {
    const h = harness();
    const skill = projectedSkill(h.sessionId, {
      source: "plugin",
      pluginId: "example@marketplace",
    ...
    for (const [toolCallId, status, output] of [
      ...
      await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId, status, rawOutput: output });
    }
  ...
  it("scopes reused native IDs by Run and preserves the first association and unknown start", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, {
    ...
    await h.collector.runtimeCapabilities.recordTool(secondRunId, {
  ...
  it("records running, success, failure and cancellation without multiplying updates", async () => {
    const h = harness();
    for (const [toolCallId, status] of [
      ...
      await h.collector.runtimeCapabilities.recordTool(h.runId, { toolCallId, kind: "read", status });
    }
    const [rank] = ranks(h.collector, "builtin_tool");
    expect(rank).toMatchObject({ calls: 4, successes: 1, failures: 2, unfinished: 1 });
  });

  it("deletes projections and activity and rejects late recreation", async () => {
    const h = harness();
    h.collector.runtimeCapabilities.recordProjection(h.runId, [projectedSkill(h.sessionId)]);
    h.collector.deleteSession(h.sessionId);

    await expect(h.collector.runtimeCapabilities.recordTool(h.runId, {
      toolCallId: "late", kind: "read", status: "completed", rawInput: { path: "README.md" }
    })).rejects.toThrow("usage_subject_deleted");
    expect(h.collector.runtimeCapabilities.stageCounts({ namespace: h.collector.namespace })).toEqual([]);
    expect(ranks(h.collector, "builtin_tool")).toEqual([]);
  });

  it("ignores updates without a stable native tool ID", async () => {
    const h = harness();
    await h.collector.runtimeCapabilities.recordTool(h.runId, { kind: "read", status: "completed", title: "dynamic" });
    expect(ranks(h.collector, "builtin_tool")).toEqual([]);
  });
});
