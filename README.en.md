...
The sidebar, Agent and Session pages open one usage analysis view with Agent, Session, date and Runtime filters. Default rankings show estimated argument, result and total content tokens per MCP tool, CLI command, Skill or plugin without requiring model HTTP forwarding. Reported model usage, observed tool content and model-input context are separate views. Unmapped models use a labeled multilingual text fallback; cache subsets and overlapping capability dimensions are not added twice.
