...
Each window selects one or more UTC weekdays, one model, an optional run-concurrency limit, and 24-hour `HH:mm` start and end times. The start is inclusive and the end is exclusive. An end earlier than its start crosses into the next UTC day, with the selected weekday representing the start day. One model may use multiple windows, such as `08:00–10:00` and `14:00–18:00` on weekdays, with a different concurrency limit in each. Earlier configured windows win when they overlap. An empty window limit inherits the Agent's normal concurrency policy; a value overrides the Agent limit but remains capped by the system-wide limit. The console provides Weekdays, Weekend, and Every day presets. The fallback model and normal concurrency apply whenever no window matches. The server refreshes the Core catalog when the policy is saved and rejects models that are no longer advertised.
