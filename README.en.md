...
The sidebar, Agent and Session pages open one usage analysis view with Agent, Session, date and Runtime filters. Default rankings compare one-time content with cumulative input occupation in captured model requests, separating definitions, arguments, and first/repeated results. Content entering multiple requests counts each time. Uncaptured requests are not inferred; the Observed content tab still shows one-time argument and result estimates without model HTTP forwarding. Known models lazily fetch and cache pinned official vocabularies on first measurement; manual profiles take precedence. Failed downloads retry in the background and leave known-model measurements pending; only unmapped models use a labeled multilingual text fallback; cache subsets and overlapping capability dimensions are not added twice.
