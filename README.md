...
自动采集中的已识别内置工具、结构化 CLI 与 Runtime 执行使用同一能力身份；组合 Shell 命令保留为 Shell，不猜测内部程序。历史调用保留首次确认的 Skill／插件归属，后续 Run 更换配置不会改写重复输入的归因。

排名切换与翻页独立刷新；能力排名的日期筛选和输入证据分页在数据库侧收窄读取范围，首次／重复归因仍基于完整历史。模型汇总和趋势复用统计逻辑，按 Session／epoch／Run 索引父子范围，避免逐个父范围扫描全量账本；仍需读取所选主体的历史计量元数据以核对累计总量和未归位用量。托管日志只解析新增记录，未写完的行保留为待重试采集。
