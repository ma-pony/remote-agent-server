...
每条规则选择一个或多个 UTC 星期，并使用明确的 24 小时制 `HH:mm` 时间。开始时间包含、结束时间不包含，例如工作日 `08:00–20:00` 命中周一至周五 UTC 08:00 至 19:59。页面提供“工作日 / 周末 / 每天”快捷选择；未命中的时间使用兜底模型。保存时服务会重新读取 Core 模型目录，并拒绝已经不在目录中的模型。
