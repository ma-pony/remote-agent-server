...
“用量分析”按 Agent、Session 和日期查询已知模型用量，并统一分析 MCP Tool、内置工具、CLI、Skill、插件及细分的提示词和对话内容。默认“模型输入上下文”视图把同一能力每次进入直接采集或根据日志重建的模型请求的定义、参数及首次／重复结果分别累计，并与一次性内容量对照；可切换“观测内容”查看运行中可见的输入、返回文本和合计，后者不需要配置 HTTP 模型转发或额外遥测。Runtime 使用 `@huggingface/tokenizers@0.2.0` 匹配手动配置或按需获取的模型词表；MCP 包装器及未知模型使用通用文本兜底。累计占用默认根据 Codex／Claude Code 原生日志中的用量边界重建，兼容现有 OAuth／API-key 登录，无需开启原生遥测。配置直接 HTTP 采集的 Provider 使用请求证据，避免与重建估算重复。模型上报用量独立显示，内容估算不能加到模型总量上，也不能跨 Skill／插件等重叠视角相加。
...
| Codex 本地记录 | 启动恢复、Run 收尾、维护前及关闭后 | Provider Session 累计范围、验证后的用量增量，以及消息／工具历史的累计输入重建；不虚构原生请求 ID |
| Claude Code 本地记录 | 启动恢复、Run 收尾、维护前及关闭后 | assistant usage、稳定消息身份和累计输入重建；不启用 Claude Code 原生遥测 |
...
会话日志中的消息、工具参数和结果只分词一次，再按请求边界累计历史内容；请求自身输出不计入它自己的输入。Codex 重复 usage 通知、Claude 同一 message.id 的修订不重复累计。压缩标记会替换旧上下文，避免继续计算已移出的结果。MCP 定义优先使用原生日志中动态加载的工具定义；没有动态工具搜索时使用 Run 冻结的允许工具配置。支持新版 Codex 分开的 namespace/name，不把定义出现当作工具执行。日志缺少历史正文或能力配置时，已有内容仍可估算，缺口体现在 partial 标记和输入差额中。没有保留正文或只有不支持的媒体时明确显示内容缺口，不能按字节数伪造原文。组合 Shell 命令不会拆成若干 CLI；结构化 MCP 镜像不会再计入 CLI。字面单命令支持常见 env／rtk／shell 包装；明确的 cat／head／tail／sed 读取和解释器脚本路径可关联已投影 Skill。仅投影的 Skill 不是已使用；插件仅在投影或快照提供明确归属时可分析。不认识的日志、工具身份和 Provider 内部调用保留未知，未知不是零。
...
默认来源链为：原生 usage 提供总账，原生消息记录提供可见上下文，运行时能力快照提供 MCP 定义及 Skill／插件关联。HTTP 请求采集是可选增强；代理认证、Provider 路由和模型选择不因默认分析改变。

原生插件通过执行器扩展启用后，其实际投影的 Skills 与托管 Skills 一起按 Run 登记，保留插件身份及缓存路径别名，Session 复用时不丢失归属。Claude 的 `Skill` 结果可能只有启动提示，实际指令另以带 `sourceToolUseID` 的消息注入；这部分正文归入 Skill／插件，随后续模型请求重复累计，不进入用户提示词。只保存计数和身份，不保存技能正文；旧 Run 缺失的投影不会用当前插件配置补造。

`/api/usage/summary` 的 `contextAnalysis` 返回已分析边界数量、重建边界数量、这些请求的上报输入、去重归因估算及两者差额。差额仅在每个已分析边界都有上报输入时提供；它包含隐藏系统内容、图片、协议包装、截断差异和分词误差，可能为负数。它与全量 Provider 输入／输出总账不是相同范围，不能以它倒推账单。Skill／插件是重叠标签，比较时同一内容位置只计一次。

重建状态不保存消息正文；每条新消息计量后合并为能力／类型／词表／首次状态分组，每个边界保存分组快照，而不是复制完整历史。来源游标与投影同事务提交，升级到 transcript/2 时按文件重新分析一次，之后断点续做；文件中没有的新信息不会凭空补齐。新 Run 的 MCP 定义配置可用于重算；同一 Session 配置未改变时复用已有快照。旧 Run 缺少的定义不使用当前配置冒充历史。

这些数值用于优化能力占用。日志没有完整呈现服务端实际上下文，因此重建统一标注为“会话重建估算”，不是精确账单；不按总输入比例给工具强行分摊差额。
...
| `configured_instructions` 配置的指令 | Session 指令快照；观测内容每 Run 计一次，累计视图每次进入上下文计一次 |
| `system_prompt` 模型请求中的系统提示词 | 请求或原生日志可见的 system/developer 内容；不可见的 Provider 隐藏指令保留在输入差额中 |
...
这些自动重算仅覆盖 Runtime 保留内容。原生日志保留时可按用量边界重建可见历史的重复输入；缺失的隐藏系统提示词、媒体或已删除正文仍不能恢复。HTTP 采集账本只保存无正文证据。历史导入快照可在保留原文时提交新 revision 重新计量。已清理的工具/消息正文不能重建，旧估算保持其原来源标记，不能直接改标为官方词表计数。
