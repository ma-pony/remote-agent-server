...
Each matching new delivery creates an independent Task/Session without inferring PR/MR conversations. Repeated IDs with identical input reuse the original Task; different input returns an idempotency conflict. GitLab checks `webhook-id`, `Idempotency-Key`, and `X-Gitlab-Webhook-UUID` in that order. For older GitLab Token requests with all three headers absent, the adapter permits an undefined delivery ID. Ingress reuses the existing message fingerprint to generate `sha256:<fingerprint>` for both the receipt and the provider-prefixed `requestId`. The fingerprint is the SHA-256 of the message containing the provider, event type, and complete `JSON.stringify` payload; JSON indentation is ignored, but object fields and array elements are not sorted. Supplied ID headers with no valid value still fail validation. GitHub and GitLab signature mode still require their respective ID headers.
