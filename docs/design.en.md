...
File-backed overview and capability queries execute in a separate Worker using a read-only connection and a consistent snapshot of the same WAL database. Read-only stores never migrate. One query runs with at most 16 queued; overload returns a sanitized 503. SQLite page cache is budgeted at 8 MiB and the Worker is released after 60 idle seconds. Shutdown rejects unfinished requests and terminates the Worker. In-memory databases use their original connection. Async queries coalesce within the same revision; writes prevent older results from entering the new cache. Conversation aggregates are reused for ranking and current-page metrics. Heavy queries still consume CPU and I/O, but synchronous aggregation stays off the management event loop.
...
`/api/usage/status` returns the revision, source/capture/failure counts and backfill progress, plus unfiltered process-local recovery phase and raw-event retirement counts, duration, skip reasons and failure time. The latter reset on restart and do not scan the database to count all pending work. It does not compute ledger totals or capability rankings. Page sections load independently. Recovery polls lightweight status and refreshes statistics on revision changes, at most once per five seconds while pending and immediately on completion. Hidden pages pause polling and resume when visible; existing pagination and request cancellation remain in place. Source management uses `UsageError.code` for domain errors and returns a sanitized 500 for unexpected internal errors.
