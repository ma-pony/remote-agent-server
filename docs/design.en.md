...
Explicit Provider tool-name adapters give recognized HTTP built-ins and CLI calls the same capability identities as Runtime events. MCP mappings take precedence and arbitrary unknown names remain unknown. Only structured outer executables identify individual CLIs; shell strings are not parsed. The per-call cache preserves the first confirmed Skill/plugin ownership and version, including an empty ownership set; later Run configuration affects new calls only.
