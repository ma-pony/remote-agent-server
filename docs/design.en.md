...
Routes validate, delegate and project responses. `UsageAnalysis` owns summary and timeseries accounting, with a query-local Session/epoch/execution containment index and detail subtotals to avoid quadratic historical scans. A timeseries query reads the ledger once and reuses one calendar formatter; no cross-query history cache is retained. Source management uses `UsageError.code` for domain errors and returns a sanitized 500 for unexpected internal errors.
