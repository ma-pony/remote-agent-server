...
Default content rankings measure each invocation's arguments and returned text once, independently of model-input exposures and Provider billing. MCP wrappers use a local text heuristic and send only counts, bytes, partial flags and bounded reason codes through the private socket. Runtime events select a tokenizer using the resolved Run model. Invocation payload projections retain counts and shared estimate metadata without copying bodies. Captured text is measured in full without per-block or per-context tokenization byte quotas. The 64K UTF-16 threshold only changes scheduling to a Worker; it never samples text or changes the counting method; binary media is excluded. These estimates do not establish how often content reached a model.
