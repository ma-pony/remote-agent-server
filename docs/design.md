...
默认累计归因从 Codex／Claude Code 原生会话日志增量重建。Codex 累计用量增加形成估算边界，重复限流快照不增加次数；Claude 使用稳定 message.id 替换同一请求的分片。请求自身输出从该次输入中排除，在后续请求才累计；已记录的压缩替换历史。MCP 运行时快照保存允许工具的定义配置，参数／结果继承具体工具及 Skill／插件归属。正文逐段计量后，仅保存按能力、内容类型、词表来源和首次／重复分类聚合的计数状态及游标；默认不再依赖 API-key 转发才能得到累计估算。重建证据标为 inferred、partial；不可见的系统／媒体内容留在同口径输入差额中，标签去重后再计算差额。已知词表等待时游标保留，后台重试。
