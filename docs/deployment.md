...
词表就绪后自动补算 Runtime 等待记录；启动时为新获得词表的模型安排旧 Runtime 兜底记录的后台重算，进度可跨重启恢复。仅能使用仍保留的 Run 输入、指令快照和工具/消息事件；HTTP 历史上下文没有原文时不可恢复。当前没有手动指定范围或暂停重算的管理入口，参见[历史重算说明](agent-usage.md#历史兜底估算与重算)。
