...
提交前复验中，完整测试与类型检查、构建同时运行时，两个既有 UI 用例在页面懒加载阶段超过 Testing Library 的 1 秒等待时间。未修改代码或放宽断言：单独运行两个相关测试文件时 31 项通过；随后独立运行完整测试时再次得到 662 项通过、10 项跳过。该现象与并行构建时的资源争用一致；交付使用独立完整测试的结果，保留此瞬时超时记录。
