...
import { ArrowLeft, Bot, Cable, ChevronLeft, ChevronRight, FolderGit2, Hash, MessageSquarePlus, MessagesSquare, Plus, Search, Trash2, XCircle } from "lucide-react";
...
export const SessionCreatePage = () => {
  ...
  const agentLoading = agents === null && agentError === "";
  const agentUnavailable = agentLoading || agentError !== "" || enabledAgents.length === 0;
  const agentDescription = agentLoading
    ? text("正在加载已启用的智能体。", "Loading enabled agents.")
    : agentError !== ""
      ? text("无法加载智能体，请检查服务后重试。", "Unable to load agents. Check the server and try again.")
      : <>{text("暂无已启用的智能体。", "No enabled agents. ")}<Link className="font-medium hover:underline" to="/agents">{text("前往智能体", "Go to agents")}</Link></>;
  const submit = async (event: FormEvent) => { event.preventDefault(); if (title.trim() === "" || agentId === "" || agentUnavailable) return; setBusy(true); setError(""); try { const created = await api<Session>("/sessions", { method: "POST", body: JSON.stringify({ title: title.trim(), agentId: Number(agentId), mcpParameters: parameterValues }) }); navigate(`/sessions/${created.id}`); } catch (reason) { setError(errorMessage(reason)); setBusy(false); } };
  return <PageContainer width="form" className="max-w-3xl"><Button asChild variant="ghost" className="mb-4"><Link to="/sessions"><ArrowLeft />{text("返回会话", "Back to sessions")}</Link></Button><PageHeader eyebrow={text("新建对话工作区", "NEW CONVERSATION WORKSPACE")} title={text("新建会话", "New session")} description={text("选择智能体后，系统会从它绑定的项目环境创建独立工作区。", "The server creates an isolated workspace from the selected agent's project environment.")} /><ErrorAlert message={agentError || error} /><Card><CardHeader><CardTitle className="flex items-center gap-2"><MessageSquarePlus className="size-5" />{text("会话信息", "Session details")}</CardTitle><CardDescription>{text("创建完成后即可发送第一条任务。", "Send the first task after creation.")}</CardDescription></CardHeader><CardContent><form className="flex flex-col gap-6" onSubmit={submit}><FieldGroup><Field><FieldLabel htmlFor="session-title">{text("会话标题", "Session title")}</FieldLabel><Input id="session-title" name="session-title" value={title} onChange={(event) => setTitle(event.target.value)} /></Field><Field data-disabled={agentUnavailable || undefined}><FieldLabel htmlFor="session-agent">{text("选择智能体", "Select agent")}</FieldLabel><NativeSelect id="session-agent" name="session-agent" className="w-full" value={agentId} disabled={agentUnavailable} onChange={(event) => setAgentId(event.target.value)}><NativeSelectOption value="" disabled>{text("请选择", "Select")}</NativeSelectOption>{enabledAgents.map((agent) => <NativeSelectOption key={agent.id} value={agent.id}>{agent.name}</NativeSelectOption>)}</NativeSelect>{agentUnavailable ? <FieldDescription>{agentDescription}</FieldDescription> : null}</Field>{parameters.length === 0 ? null : <div className="rounded-lg border bg-muted/20 p-4"><p className="mb-4 text-sm font-medium">{text("MCP 会话参数", "MCP session parameters")}</p><FieldGroup>{parameters.map((parameter) => <Field key={parameter.id}><FieldLabel htmlFor={`create-parameter-${parameter.key}`}>{parameter.label}{parameter.required ? text("（必填）", " (required)") : ""}</FieldLabel><Input id={`create-parameter-${parameter.key}`} name={`session-parameter-${parameter.key}`} type={parameter.secret ? "password" : "text"} value={parameterValues[parameter.key] ?? ""} required={parameter.required} onChange={(event) => setParameterValues((current) => ({ ...current, [parameter.key]: event.target.value }))} />{parameter.description === null ? null : <FieldDescription>{parameter.description}</FieldDescription>}</Field>)}</FieldGroup></div>}<div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button asChild variant="outline"><Link to="/sessions">{text("取消", "Cancel")}</Link></Button><Button type="submit" disabled={busy || title.trim() === "" || agentUnavailable || agentId === ""}>{busy ? text("创建中…", "Creating…") : text("创建会话", "Create session")}</Button></div></FieldGroup></form></CardContent></Card></PageContainer>;
};
