...
import { ArrowLeft, MessageSquarePlus, Plus, Search, XCircle } from "lucide-react";
...
const ErrorAlert = ({ message }: { message: string }) => message === "" ? null : <Alert variant="destructive"><XCircle /><AlertTitle>加载失败</AlertTitle><AlertDescription>{message}</AlertDescription></Alert>;
...
export const SessionListPage = () => {
  ...
  return <div className="mx-auto w-full max-w-6xl p-4 sm:p-6 lg:p-8"><PageHeader eyebrow="CONVERSATION WORKSPACES" title="Session" description="每个 Session 保留独立 Workspace，并可在同一 Agent 上继续多轮任务。" action={<Button asChild><Link to="/sessions/new"><Plus />新建 Session</Link></Button>} /><ErrorAlert message={error} /><div className="mb-5 flex max-w-sm items-center gap-2 rounded-lg border bg-card px-3"><Search className="size-4 text-muted-foreground" /><Input aria-label="搜索 Session" className="border-0 bg-transparent shadow-none focus-visible:ring-0" placeholder="按标题搜索" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
    {sessions === null ? <div className="flex flex-col gap-3">{[0, 1, 2].map((item) => <Skeleton key={item} className="h-24" />)}</div> : visible.length === 0 ? <Card className="border-dashed"><CardContent className="py-16 text-center text-muted-foreground">{sessions.length === 0 ? "暂无 Session，创建一个开始对话。" : "没有匹配的 Session。"}</CardContent></Card> : <div className="divide-y rounded-xl border bg-card">{visible.map((session) => <Link key={session.id} to={`/sessions/${session.id}`} aria-label={session.title} className="flex flex-col gap-3 p-5 transition-colors hover:bg-muted/50 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h2 className="truncate font-medium">{session.title}</h2><p className="mt-1 text-sm text-muted-foreground">{names.get(session.agentId) ?? session.agentId}</p></div><div className="flex items-center gap-4"><Badge variant={session.status === "running" ? "default" : "secondary"}>{session.status === "running" ? "运行中" : "空闲"}</Badge><time className="text-sm text-muted-foreground" dateTime={session.updatedAt}>{displayTime(session.updatedAt)}</time></div></Link>)}</div>}
  </div>;
};
