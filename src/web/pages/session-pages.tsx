...
import { ArrowLeft, Bot, Cable, FolderGit2, Hash, MessageSquarePlus, MessagesSquare, Plus, Search, Trash2, XCircle } from "lucide-react";
...
export const SessionCreatePage = () => {
  ...
  const [agentId, setAgentId] = useState("");
  const [parameterResult, setParameterResult] = useState<Page<AgentSessionParameter> | null>(null);
  const parameters = parameterResult?.items ?? [];
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => {setParameterPage(1); setParameterValues(Object.create(null));}, [agentId]);
  useEffect(() => {
    setParameterResult(null);
    if (agentId === "") return;
    const controller = new AbortController();
    void api<Page<AgentSessionParameter>>(`/agents/${agentId}/session-parameters?page=${parameterPage}&pageSize=20`, {signal: controller.signal})
      .then(setParameterResult).catch(reason => {if (!controller.signal.aborted) setError(errorMessage(reason));});
    return () => controller.abort();
  }, [agentId, parameterPage]);
  const agentUnavailable = agentId === "";
  const agentDescription = <>{text("请选择已启用的智能体。", "Choose an enabled agent. ")}<Link className="font-medium hover:underline" to="/agents">{text("前往智能体", "Go to agents")}</Link></>;
