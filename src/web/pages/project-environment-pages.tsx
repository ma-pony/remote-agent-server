...
export const ProjectEnvironmentListPage = () => {
  ...
  return <div className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-8">
    ...
    {items === null ? <div className="grid gap-4 md:grid-cols-2">{[0, 1].map((item) => <Skeleton key={item} className="h-48" />)}</div>
      ...
      : <div className="grid gap-4 md:grid-cols-2">{items.map((environment) => { const status = revisionStatus(environment); return <Card key={environment.id} className="transition-colors hover:border-primary/50"><CardHeader><div className="flex items-start justify-between gap-4"><div><CardTitle><Link className="hover:underline" to={`/project-environments/${environment.id}`}>{environment.name}</Link></CardTitle><CardDescription className="mt-2">{environment.repositories.length} 个项目</CardDescription></div><Badge variant={status.variant}>{status.label}</Badge></div></CardHeader><CardContent className="flex flex-col gap-2 text-sm text-muted-foreground"><p><span className="font-medium text-foreground">当前版本：</span>{environment.currentRevisionId?.slice(0, 8) ?? "—"}</p><p><span className="font-medium text-foreground">最近检查：</span>{environment.lastCheckedAt === null ? "尚未检查" : new Date(environment.lastCheckedAt).toLocaleString("zh-CN")}</p></CardContent></Card>; })}</div>}
  </div>;
};
...
export const ProjectEnvironmentDetailLayout = () => {
  ...
  useEffect(() => {
    if (environment?.latestRevision?.status !== "preparing") return;
...
export const ProjectEnvironmentOverviewPage = () => {
  ...
  const [busy, setBusy] = useState(false);
  const check = async () => { setBusy(true); setError(""); try { await api(`/project-environments/${environment.id}/check`, { method: "POST", body: "{}" }); await reload(); } catch (reason) { setError(errorMessage(reason)); } finally { setBusy(false); } };
  const revision = environment.latestRevision;
  return <div className="flex flex-col gap-5"><ErrorAlert message={error} /><div className="grid gap-4 md:grid-cols-3"><Card><CardHeader><CardDescription>项目数量</CardDescription><CardTitle className="text-3xl">{environment.repositories.length}</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>当前版本</CardDescription><CardTitle className="font-mono text-lg">{environment.currentRevisionId?.slice(0, 8) ?? "—"}</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>最近检查</CardDescription><CardTitle className="text-base">{environment.lastCheckedAt === null ? "尚未检查" : new Date(environment.lastCheckedAt).toLocaleString("zh-CN")}</CardTitle></CardHeader></Card></div>
    <Card><CardHeader><CardTitle>执行环境准备</CardTitle><CardDescription>按当前项目配置创建新的 APFS/Btrfs 基础版本。系统每 3 小时自动检查，也可以手动触发。</CardDescription></CardHeader><CardContent><Button onClick={() => void check()} disabled={busy || revision?.status === "preparing"}><RefreshCw className={busy || revision?.status === "preparing" ? "animate-spin" : ""} />{revision?.status === "preparing" ? "准备中…" : "立即检查"}</Button></CardContent></Card>
...
export const ProjectEnvironmentRepositoriesPage = () => {
  ...
  return <div className="flex flex-col gap-5"><ErrorAlert message={error} /><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">项目清单</h2><p className="text-sm text-muted-foreground">每个项目会被准备到同一个基础 Workspace。</p></div><RepositoryDialog environmentId={environment.id} repository={null} open={editing === null} onOpenChange={(open) => setEditing(open ? null : undefined)} disabled={preparing} onSaved={async () => { setEditing(undefined); await reload(); }} onError={setError}><Button disabled={preparing}><Plus />添加项目</Button></RepositoryDialog></div>
    {environment.repositories.length === 0 ? <Card className="border-dashed"><CardContent className="py-14 text-center text-muted-foreground">尚未添加项目。添加后系统会自动开始准备。</CardContent></Card> : <div className="flex flex-col gap-3">{environment.repositories.map((repository) => <Card key={repository.id}><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><GitBranch className="size-4" /><h3 className="font-medium">{repository.name}</h3></div><p className="mt-2 truncate font-mono text-xs text-muted-foreground">{repository.gitUrl}</p><p className="mt-1 text-sm text-muted-foreground">准备命令：{repository.prepareCommand ?? "无"}</p></div><div className="flex gap-2"><RepositoryDialog environmentId={environment.id} repository={repository} open={editing?.id === repository.id} onOpenChange={(open) => setEditing(open ? repository : undefined)} disabled={preparing} onSaved={async () => { setEditing(undefined); await reload(); }} onError={setError}><Button size="sm" variant="outline" disabled={preparing}><Pencil />编辑</Button></RepositoryDialog><Button size="sm" variant="ghost" disabled={preparing} onClick={() => void (async () => { try { await api(`/project-environments/${environment.id}/repositories/${repository.id}`, { method: "DELETE" }); await reload(); } catch (reason) { setError(errorMessage(reason)); } })()}><Trash2 />移除</Button></div></CardContent></Card>)}</div>}
