...
export const SessionPage = ({ sessionId }: { sessionId: string }) => {
  ...
  return (
    <PageContainer>
      ...
      <PageHeader eyebrow={text(`会话 / ${initialLoading ? "加载中" : session !== null && session.storageCleanedAt != null ? "已归档" : activeRunId !== null ? "运行中" : "空闲"}`, `SESSION / ${initialLoading ? "LOADING" : session !== null && session.storageCleanedAt != null ? "ARCHIVED" : activeRunId !== null ? "RUNNING" : "IDLE"}`)} title={session?.title ?? text("加载会话…", "Loading session…")} description={agentName === "" ? text("正在读取智能体…", "Loading agent…") : text(`智能体 · ${agentName}`, `Agent · ${agentName}`)} action={<div className="flex flex-wrap items-center gap-2">{session === null ? null : <><Button asChild size="sm" variant="outline"><Link to={`/usage?agentId=${session.agentId}&sessionId=${session.id}&range=all`}><BarChart3 />{text("用量分析", "Usage analysis")}</Link></Button><Button asChild size="sm" variant="outline"><Link to={`/sessions/${session.id}/settings`}><Settings2 />{text("设置", "Settings")}</Link></Button></>}<Badge variant={session !== null && session.storageCleanedAt != null ? "outline" : activeRunId === null ? "secondary" : "default"}>{session !== null && session.storageCleanedAt != null ? text("存储已清理", "Storage cleaned") : activeRunId === null ? text("空闲", "Idle") : text("活动中", "Active")}</Badge>{session === null ? null : <SessionDeleteDialog session={activeRunId === null ? session : { ...session, status: "running" }} onDeleted={() => navigate("/sessions")} onError={setError} />}</div>} />
      {session === null ? null : <div className="mt-6"><LedgerUsageSummaryCard agentId={session.agentId} sessionId={session.id}
        refreshKey={views.map(({ run }) => `${run.id}:${run.status}`).join(",")} headingLevel={2}
        title={text("累计 Token 用量", "Cumulative token usage")} /></div>}
