...
export const AgentSkillsPage = () => {
  ...
  return <div className="flex flex-col gap-5"><ErrorAlert message={error} />
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><SkillSourcesDialog agentId={agent.id} disabled={busy !== ""} onChanged={reload} /><Button variant="outline" asChild><label><Upload />{busy === "upload" ? text("上传中…", "Uploading…") : text("上传 ZIP 到共享库", "Upload ZIP to shared library")}<input className="sr-only" type="file" accept=".zip,application/zip" disabled={busy !== ""} onChange={(event) => void upload(event.target.files?.[0], event.currentTarget)} /></label></Button></div>
    </div>
