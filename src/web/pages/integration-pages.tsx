...
const CopyableCode = ({ title, value }: { title: string; value: string }) => {
  ...
  return <div className="overflow-hidden rounded-lg border bg-sidebar text-sidebar-foreground">
...
export const IntegrationEndpointDetailLayout = () => {
  ...
  if (endpoint === null) return <PageContainer><ErrorAlert message={error} /><Skeleton className="h-12 w-72" /><Skeleton className="mt-8 h-64" /></PageContainer>;
...
export const IntegrationEndpointUsagePage = () => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (busy || attachmentDraft.reading || !endpoint.enabled || (message.trim() === "" && attachmentDraft.attachments.length === 0)) return;
    setBusy(true);
    setError("");
    try {
      ...
      const created = await integrationApi.createTestTask(endpoint.id, {
        ...
        ...(attachmentDraft.attachments.length === 0 ? {} : { attachments: attachmentDraft.attachments }),
        parameters
      });
  ...
  return <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(20rem,0.8fr)]">
    <div className="flex min-w-0 flex-col gap-5">
      ...
      <Card><CardHeader><CardTitle>{text("请求参数", "Request parameters")}</CardTitle><CardDescription>{text("参数名称区分大小写。相同请求标识重复提交时会返回同一个任务。", "Parameter names are case-sensitive. Reusing the same request ID returns the same task.")}</CardDescription></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[42rem] text-left text-sm"><thead className="border-b text-xs text-muted-foreground"><tr><th className="pb-3 font-medium">{text("参数", "Parameter")}</th><th className="pb-3 font-medium">{text("必填", "Required")}</th><th className="pb-3 font-medium">{text("说明", "Description")}</th></tr></thead><tbody className="divide-y"><tr><td className="py-3"><code>requestId</code></td><td className="py-3">{text("是", "Yes")}</td><td className="py-3 text-muted-foreground">{text("调用方生成的唯一请求标识，用于安全重试。", "A caller-generated unique ID used for safe retries.")}</td></tr><tr><td className="py-3"><code>message</code></td><td className="py-3">{text("无附件时必填", "Without attachments")}</td><td className="py-3 text-muted-foreground">{text("发送给智能体的任务内容；有附件时可省略。", "The task message; optional when attachments are included.")}</td></tr><tr><td className="py-3"><code>attachments</code></td><td className="py-3">{text("否", "No")}</td><td className="py-3 text-muted-foreground">{text("图片或文件数组，每项包含 name、mediaType 和 base64 编码的 data。最多 8 个，总计 20 MiB。", "Images or files, each with name, mediaType, and base64 data. Up to 8 attachments and 20 MiB total.")}</td></tr><tr><td className="py-3"><code>conversationKey</code></td><td className="py-3">{text("否", "No")}</td><td className="py-3 text-muted-foreground">{text("相同对话标识会接续同一个业务对话；不传则每次创建独立会话。", "The same key continues one business conversation; omit it to create a new session per task.")}</td></tr><tr><td className="py-3"><code>parameters</code></td><td className="py-3">{text("否", "No")}</td><td className="py-3 text-muted-foreground">{text("当前端点声明的动态业务参数对象。", "Dynamic business parameters declared by this endpoint.")}</td></tr>{requestMappings.map((mapping) => { const definition = definitionByKey.get(mapping.parameterKey); return <tr key={mapping.parameterKey}><td className="py-3"><code>parameters.{mapping.requestKey}</code><span className="mt-1 block text-xs text-muted-foreground">{definition?.label ?? mapping.parameterKey}</span></td><td className="py-3">{definition?.required ? text("是", "Yes") : text("否", "No")}</td><td className="py-3 text-muted-foreground">{definition?.description ?? text("传入当前任务使用的参数值。", "The parameter value used by this task.")}</td></tr>; })}</tbody></table></CardContent></Card>
...
const WebhookEditorDialog = ({ endpointId, webhook, onSaved, onError }: {
  endpointId: number;
  webhook?: IntegrationWebhook;
  onSaved(webhook: IntegrationWebhook, secret?: string): void;
  ...
  const [timeoutSeconds, setTimeoutSeconds] = useState(webhook?.timeoutSeconds ?? 10);
  const [busy, setBusy] = useState(false);
  const reset = () => {
    setName(webhook?.name ?? ""); setUrl(webhook?.url ?? ""); setEvents(webhook?.events ?? []);
    setHeaderName(""); setHeaderValue(""); setClearHeaders(false);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (webhook === undefined) {
        ...
        onSaved(created.webhook, created.signingSecret);
      } else {
        const input: Partial<IntegrationWebhookInput> = {
          name: name.trim(), url: url.trim(), events, timeoutSeconds,
          ...(clearHeaders
            ? { headers: {} }
            : headerName.trim() === "" ? {} : { headers: { [headerName.trim()]: headerValue } })
        };
        onSaved(await integrationApi.updateWebhook(endpointId, webhook.id, input));
      }
      setOpen(false); reset();
    } catch (reason) { onError(errorMessage(reason)); } finally { setBusy(false); }
  };
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (nextOpen) reset(); setOpen(nextOpen); }}><DialogTrigger asChild>{editing ? <Button size="sm" variant="outline" aria-label={text(`编辑 ${webhook.name}`, `Edit ${webhook.name}`)}><Pencil />{text("编辑", "Edit")}</Button> : <Button><Plus />{text("新建事件回调", "New webhook")}</Button>}</DialogTrigger><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><form onSubmit={submit}><DialogHeader><DialogTitle>{editing ? text("编辑事件回调", "Edit webhook") : text("新建事件回调", "New webhook")}</DialogTitle><DialogDescription>{editing ? text("签名密钥不会改变。留空请求头会保留现有配置。", "The signing secret remains unchanged. Leave headers blank to preserve the current configuration.") : text("固定使用 HTTP POST。签名密钥只会在创建成功后展示一次。", "Webhooks use HTTP POST. The signing secret is shown only once after creation.")}</DialogDescription></DialogHeader><FieldGroup className="py-5"><Field><FieldLabel htmlFor={`webhook-name-${webhook?.id ?? "new"}`}>{text("回调名称", "Webhook name")}</FieldLabel><Input id={`webhook-name-${webhook?.id ?? "new"}`} required value={name} onChange={(event) => setName(event.target.value)} /></Field><Field><FieldLabel htmlFor={`webhook-url-${webhook?.id ?? "new"}`}>{text("回调地址", "Webhook URL")}</FieldLabel><Input id={`webhook-url-${webhook?.id ?? "new"}`} required type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://example.com/callback" /></Field><Field><FieldLabel>{text("订阅事件", "Subscribed events")}</FieldLabel><div className="grid gap-2 rounded-lg border p-3 sm:grid-cols-2">{webhookEvents.map((item) => <label key={item.value} className="flex items-start gap-2 text-sm"><input aria-label={item.value} type="checkbox" checked={events.includes(item.value)} onChange={() => toggleEvent(item.value)} className="mt-0.5" /><span><span className="font-mono text-xs">{item.value}</span><span className="block text-xs text-muted-foreground">{text(...item.labels)}</span></span></label>)}</div><FieldDescription>{text("至少选择一个事件。智能体思考内容不会通过事件回调外发。", "Select at least one event. Agent thoughts are never sent through webhooks.")}</FieldDescription></Field>{editing && webhook.headers.length > 0 ? <Field><FieldLabel>{text("已配置请求头", "Configured headers")}</FieldLabel><div className="flex flex-wrap gap-2">{webhook.headers.map((header) => <Badge key={header.name} variant="outline">{text(`${header.name} 已配置`, `${header.name} configured`)}</Badge>)}</div><FieldDescription>{text("不填写下面的新请求头时会保留这些值。", "These values are preserved unless a replacement header is provided below.")}</FieldDescription></Field> : null}<div className="grid gap-4 sm:grid-cols-2"><Field><FieldLabel htmlFor={`webhook-header-name-${webhook?.id ?? "new"}`}>{text(editing ? "替换请求头名称（可选）" : "请求头名称（可选）", editing ? "Replacement header name (optional)" : "Header name (optional)")}</FieldLabel><Input id={`webhook-header-name-${webhook?.id ?? "new"}`} value={headerName} disabled={clearHeaders} onChange={(event) => setHeaderName(event.target.value)} placeholder="Authorization" /></Field><Field><FieldLabel htmlFor={`webhook-header-value-${webhook?.id ?? "new"}`}>{text("请求头值", "Header value")}</FieldLabel><Input id={`webhook-header-value-${webhook?.id ?? "new"}`} type="password" value={headerValue} disabled={clearHeaders} onChange={(event) => setHeaderValue(event.target.value)} autoComplete="off" /></Field></div>{editing && webhook.headers.length > 0 ? <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={clearHeaders} onChange={(event) => setClearHeaders(event.target.checked)} />{text("清除已有请求头", "Clear configured headers")}</label> : null}<Field><FieldLabel htmlFor={`webhook-timeout-${webhook?.id ?? "new"}`}>{text("超时秒数", "Timeout in seconds")}</FieldLabel><Input id={`webhook-timeout-${webhook?.id ?? "new"}`} type="number" min={1} max={60} value={timeoutSeconds} onChange={(event) => setTimeoutSeconds(Number(event.target.value))} /></Field></FieldGroup><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>{text("取消", "Cancel")}</Button><Button type="submit" disabled={busy || events.length === 0 || (headerName.trim() !== "" && headerValue === "")}>{busy ? text(editing ? "保存中…" : "创建中…", editing ? "Saving…" : "Creating…") : text(editing ? "保存修改" : "创建事件回调", editing ? "Save changes" : "Create webhook")}</Button></DialogFooter></form></DialogContent></Dialog>;
};
...
export const IntegrationEndpointWebhooksPage = () => {
  ...
  const rotateSigningSecret = async (webhook: IntegrationWebhook) => {
    setBusyId(webhook.id); setError("");
    try {
      const rotated = await integrationApi.rotateWebhookSecret(endpoint.id, webhook.id);
      setWebhooks((current) => (current ?? []).map((item) => item.id === rotated.webhook.id ? rotated.webhook : item));
      setSigningSecretRotated(true);
...
export const IntegrationTaskDetailPage = () => {
  ...
  if (task === null) return <PageContainer><ErrorAlert message={error} /><Skeleton className="h-12 w-80" /><Skeleton className="mt-8 h-80" /></PageContainer>;
