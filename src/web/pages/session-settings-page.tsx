...
export const SessionSettingsPage = () => {
  ...
  if (session === null && error === "") return <PageContainer width="form" className="max-w-3xl"><Skeleton className="h-10 w-72" /><Skeleton className="mt-8 h-72" /></PageContainer>;
  return <PageContainer width="form" className="max-w-3xl"><Button variant="ghost" asChild className="mb-4"><Link to={`/sessions/${id}`}><ArrowLeft />{text("返回对话", "Back to conversation")}</Link></Button><PageHeader eyebrow={text("会话设置", "SESSION SETTINGS")} title={session?.title ?? text("会话设置", "Session settings")} description={text("这些参数只对当前会话生效。", "These parameters apply only to this session.")} action={session === null ? null : <Badge variant={session.storageCleanedAt != null ? "outline" : session.status === "idle" ? "secondary" : "default"}>{session.storageCleanedAt != null ? text("存储已清理", "Storage cleaned") : session.status === "idle" ? text("空闲", "Idle") : text("运行中", "Running")}</Badge>} />
