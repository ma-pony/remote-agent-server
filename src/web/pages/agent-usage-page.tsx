...
type SummaryResponse = UsageSummary & UsageMetadata & { sourceCounts?: Record<string, number>;
  contextAnalysis?: { requests: number; reconstructedRequests: number; requestsWithReportedInput: number;
    reportedInputTokens: number | null; estimatedInputTokens: number | null; differenceTokens: number | null } };
...
export const AgentUsagePage = () => {
  ...
  return <PageContainer width="wide">
    ...
    {summary === null ? error === "" ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" role="status" aria-label={text("正在加载用量分析", "Loading usage analysis")}>{[0, 1, 2, 3].map((item) => <Skeleton key={item} className="h-32" />)}</div> : null : <>
      ...
      {view === "context" && summary?.contextAnalysis && summary.contextAnalysis.requests > 0 ? <Card>
        <CardHeader><CardTitle>{text("输入占用对照", "Input comparison")}</CardTitle><CardDescription>{text(
          `已分析 ${summary.contextAnalysis.requests} 个请求边界，其中 ${summary.contextAnalysis.reconstructedRequests} 个来自会话日志重建。以下数字仅比较这些请求的输入，已去除技能、插件关联标签的重复计量。`,
          `${summary.contextAnalysis.requests} request boundaries analyzed; ${summary.contextAnalysis.reconstructedRequests} reconstructed from transcripts. This comparison covers their input only and deduplicates skill and plugin tags.`)}</CardDescription></CardHeader>
        <CardContent><div className="grid gap-4 sm:grid-cols-3">
          <MetricCard label={text("对应请求上报输入", "Reported input for analyzed requests")} value={formatNumber(summary.contextAnalysis.reportedInputTokens)} />
          <MetricCard label={text("已归因输入估算", "Attributed input estimate")} value={formatNumber(summary.contextAnalysis.estimatedInputTokens)} />
          <MetricCard label={text("输入差额", "Input difference")} value={formatNumber(summary.contextAnalysis.differenceTokens)} description={text("包含未记录的系统内容、媒体、协议开销及估算误差；负数表示估算偏大。", "Includes unrecorded system content, media, protocol overhead and estimation error; negative means overestimation.")} />
        </div></CardContent>
      </Card> : null}
      ...
      {ranking === null ? <Skeleton className="h-48" aria-label={text("正在加载排名", "Loading rankings")} /> : empty ? filteredEmpty
        ...
        : <EmptyState icon={SearchX} title={text("尚无用量记录", "No usage records yet")} description={text("受管会话会自动发现 Provider 日志，统计上报用量并重建累计输入占用。运行一次会话后即可查看分析。", "Managed sessions automatically discover provider transcripts, collect reported usage and reconstruct cumulative input. Run a session to see the analysis.")} action={<Button type="button" onClick={() => setGuideOpen(true)}>{text("查看接入方法", "View setup guide")}</Button>} />
  ...
  return <Card>
    <CardHeader><CardTitle>{view === "content" ? text("能力内容估算", "Capability content estimates") : text("累计能力占用估算", "Cumulative capability input estimates")}</CardTitle>
      <CardDescription>{view === "content"
        ...
        : text("根据会话日志中的请求边界重建上下文，同一能力内容每被再次带入一次请求，就累计一次；已记录的压缩会替换旧上下文。直接采集的请求优先使用实际输入。", "Reconstruct context at request boundaries in transcripts, counting content again each time it is reused. Recorded compaction replaces old context. Direct request capture uses the observed input when configured.")}</CardDescription>
            ...
            <tbody className="divide-y">{ranking.items.map((row) => {
              ...
              return <tr key={`${row.capability.kind}:${row.capability.serverId ?? ""}:${row.capability.id}`}>
                <td className="py-3">
                  ...
                  {(row.attributionEvidence?.inferred ?? 0) > 0 ? <Badge variant="outline">{text("会话重建估算", "Transcript estimate")}</Badge> : null}
...
const GuideSheet = ({ open, onOpenChange }: { open: boolean; onOpenChange(open: boolean): void }) => {
  const { text } = useI18n();
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="overflow-y-auto sm:max-w-lg"><SheetHeader><SheetTitle>{text("接入用量来源", "Connect a usage source")}</SheetTitle><SheetDescription>{text("Codex 和 Claude Code 默认从会话日志自动统计上报用量与累计输入占用，无需更改认证。也可配置 USAGE_CAPTURE_UPSTREAMS 使用直接请求采集。", "Codex and Claude Code automatically analyze usage and cumulative input from transcripts without changing authentication. USAGE_CAPTURE_UPSTREAMS optionally enables direct request capture.")}</SheetDescription></SheetHeader><div className="flex flex-col gap-4 px-4 pb-6"><Card><CardHeader><CardTitle>{text("可用来源", "Available sources")}</CardTitle><CardDescription>{text("工具活动提供一次性内容量；会话日志提供上报用量及累计输入重建；运行时能力快照补充 MCP 定义、技能与插件关联。上下文快照也支持手动导入。", "Tool activity measures content once; transcripts provide reported usage and cumulative input reconstruction. Runtime snapshots add MCP definitions and skill/plugin associations. Context Snapshots also support manual imports.")}</CardDescription></CardHeader><CardContent className="flex flex-col gap-3 text-sm"><p><Badge variant="outline">Codex log</Badge> <code>codex_log</code></p><p><Badge variant="outline">Claude log</Badge> <code>claude_log</code></p><p><Badge variant="outline">{text("上下文快照", "Context Snapshot")}</Badge> <code>context_snapshot</code> · <code>context-snapshot-v1</code></p></CardContent></Card><Alert><Database /><AlertTitle>{text("采集后再核验", "Verify after collection")}</AlertTitle><AlertDescription>{text("注册来源后，在数据来源列表点击“重新采集”。完整状态需要模型请求用量和上下文暴露都被来源覆盖。", "After registering a source, select Collect in the data source list. Complete analysis requires source coverage for both model-request usage and context exposure.")}</AlertDescription></Alert></div></SheetContent></Sheet>;
};
