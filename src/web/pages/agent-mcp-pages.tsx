...
export const AgentMcpPage = () => {
  ...
  const toggleCore = async (server: AgentMcpServerSummary) => {
    setBusy(`core-${server.id}`); setError("");
    try {
      const updated = await api<AgentMcpServerSummary>(`/agents/${id}/mcp-servers/${server.id}/core`, {
        method: "PATCH", body: JSON.stringify({ core: !server.core })
      });
      setServers((items) => (items ?? []).map((item) => item.id === updated.id ? updated : item));
    } catch (reason) { setError(errorMessage(reason)); } finally { setBusy(""); }
  };
