    ...
    try {
      this.db.transaction(() => {
        ...
        const servers = this.db.prepare(`
          SELECT id, source_mcp_server_id, name, transport, enabled, core, url, command, check_timeout_seconds
          FROM agent_mcp_servers WHERE agent_id = ? ORDER BY created_at ASC, id ASC
        `).all(id) as Array<{
          id: number; source_mcp_server_id: number | null; name: string; transport: string; enabled: number; core: number; url: string | null;
        ...
        for (const server of servers) {
          const newServerId = insertedId(this.db.prepare(`
            INSERT INTO agent_mcp_servers
              (agent_id, source_mcp_server_id, name, transport, enabled, core, url, command,
