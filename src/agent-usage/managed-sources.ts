...
export class ManagedUsageSources {
  ...
  private async discover(sessionId: number): Promise<void> {
    ...
    const existing = new Set(this.collector.sources.listSources(this.collector.namespace).map((source) => source.sourceKey));
    let visited = 0;
    const walk = async (directory: string, depth: number): Promise<void> => {
      if (depth > 8) throw new Error("usage_discovery_limit");
      let entries;
      try { entries = await readdir(directory, { withFileTypes: true }); }
      catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return; throw error; }
      for (const entry of entries) {
        if (++visited > 5000) throw new Error("usage_discovery_limit");
  ...
  async register(request: SourceRegistration): Promise<{ source: SourceRecord; created: boolean }> {
    const mappings = request.mappings.map((mapping) => {
      ...
      if (subject.state !== "active") throw new Error("usage_collection_pending");
      ...
      if (mapping.providerEpochId !== currentEpoch && knownEpoch === undefined) throw new Error("usage_epoch_mismatch");
      if ("providerSessionRef" in request.inputRef && (request.inputRef.providerSessionRef !== mapping.sessionId
        || session.provider_session_id !== mapping.sourceSessionKey)) throw new Error("usage_mapping_mismatch");
    ...
    const existed = this.collector.sources.listSources(this.collector.namespace).some((source) => source.sourceKey === request.sourceKey);
    ...
    for (const mapping of mappings) {
      ...
      if (this.collector.sources.maintenance(binding)) throw new Error("usage_collection_pending");
    }
    return { source: this.collector.sources.registerSource(config), created: !existed };
  }

  private session(id: string): HostSession {
    if (!/^[1-9]\d*$/.test(id) || !Number.isSafeInteger(Number(id))) throw new Error("usage_session_not_found");
    ...
    if (!row) throw new Error("usage_session_not_found");
    return row;
  }
  ...
  private async resolve(input: Record<string, string>, kind: SourceRegistration["kind"]): Promise<string> {
    ...
    if (!path || isAbsolute(path) || path.includes("\0") || path.split(/[\\/]/).includes("..")) throw new Error("usage_source_path_denied");
    let root: string;
    if (input.importRootId !== undefined && input.providerSessionRef === undefined) {
      ...
      if (!configured) throw new Error("usage_source_path_denied");
      root = configured;
      ...
      if (kind === "codex_log" && session.provider === "codex") {
        ...
        if (path.split(/[\\/]/).at(-1) !== `${session.provider_session_id}.jsonl`) throw new Error("usage_source_path_denied");
        root = join(this.config.dataDir, "agents", String(session.agent_id), "provider-home", "claude");
      } else throw new Error("usage_source_path_denied");
    } else throw new Error("usage_source_path_denied");
    try {
      ...
      return file;
    } catch { throw new Error("usage_source_path_denied"); }
  }
}
